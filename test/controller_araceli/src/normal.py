...
class Normal(object):
    ...
    def __str__ (self):
        return "Normal ( mu = {0} , sigma = {1} , elliptical = {2} )".format(self._mu.tolist(), self._sigma.tolist(), self._ellip)
    ...
    def addTo (self, grids, result):
        if self._ellip:
            ...
            Normal._calcExp(grids, self._mu, (1.0 / np.sqrt(((Normal.two_pi) ** self._dimensions) *
                                                            np.linalg.det(self._sigma))), np.linalg.inv(self._sigma), result)
